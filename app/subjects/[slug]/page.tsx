...
export default function SubjectPage({
  ...
  const isPlaceholder = subject.moodleQuizUrl.includes("YOUR-SITE.moodlecloud.com");
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-16">
      ...
      <p className="mt-3 text-zinc-700 dark:text-zinc-300 leading-7">
        You may be asked to log in or enroll before attempting the quiz. Make
        sure you have a stable internet connection before starting.
      </p>

      {isPlaceholder ? (
        <p className="mt-8 text-sm text-zinc-600 dark:text-zinc-400">
          Quiz link for <strong>{subject.name}</strong> is coming soon.
        </p>
      ) : (
        <a
